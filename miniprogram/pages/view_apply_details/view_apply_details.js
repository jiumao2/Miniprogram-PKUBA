...
Page({
  ...
  checkRequestValidation(){
    ...
    {
      if (this.data.request_detail.type==1 || (this.data.request_detail.type==3 && this.data.request_detail.teams_to_vote.length==this.data.request_detail.voted_accept.length)){
        if (this.data.request_detail.type==1){
          var new_state = 2
          var addition_words = ''
        }
        else if (this.data.request_detail.type==3){
          var new_state = 4
          var addition_words = '组内'
        }
          ...
          success: res => {
            ...
            wx.cloud.callFunction({
              ...
              data:{
                text: text_email,
                attachment: 0,
                subject: app.globalData.TYPES[app.globalData.request_detail.type-1]+'申请 '+ app.globalData.request_detail.home_team + " VS "+ app.globalData.request_detail.away_team + " (" + addition_words + "协商成功)"
              },
              success: res =>{
                console.log(res)
                wx.navigateBack()
  ...
  accept(){
    ...
    if (this.data.request_detail.is_vote){
      ...
      wx.cloud.callFunction({
        ...
        success: res=>{
          console.log(res)
          this.data.request_detail.voted_accept = voted_accept
